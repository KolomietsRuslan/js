$(document).ready(function () {
    ...
    function Skittles(options) {
        this.id = Math.trunc(Math.random() * 1000);
        this.x = options.x;
        this.y = options.y;
        this.width = Math.min(options.width, 40);
        this.height = Math.min(options.height, 70);
        this.width = Math.max(this.width, 10);
        this.height = Math.max(this.height, 10);

        this.createView = function () {
            let result = document.createElement("div");
            result.idProperty = 'skit';
            result.style.cssText = `
              position: absolute;
              left: 500px;
              top: 10px;
    ...
    function checkCollision(objectA, objectB) {
        if (objectA !== objectB) {
            ...
            if (objectA instanceof Skittles) {
                skittle = objectA;
            } else if (objectB instanceof Skittles) {
                skittle = objectB;
            }
            ...
            if (ball && skittle) {
                 if (
                    (ball.x + ball.diametr === skittle.x ||
                    ball.x === skittle.x + skittle.width) &&
                    ball.y > skittle.y &&
                    ball.y < skittle.y + skittle.height
                ) {
                    ball.invertDirectionX();
                    skittle.width = 0;
                    skittle.height = 0;
